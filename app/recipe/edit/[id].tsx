...
export default function EditRecipeScreen() {
  ...
  const loadRecipe = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        setCategory(recipe.category || 'vietnamese');
        setDifficulty(recipe.difficulty || 'Dễ');
        ...
        if (recipe.images && recipe.images.length > 0) {
          ...
          setOriginalImages([recipe.image]);
        }
        // Load videos
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload nhiều ảnh (chỉ upload ảnh mới, không phải URL)
      ...
      console.log('📤 Existing images (URLs):', existingImages.length);
      ...
      if (newImages.length > 0) {
        ...
        const firstImageUri = Platform.OS === 'android' ? newImages[0] : newImages[0].replace('file://', '');
      ...
      // Gửi danh sách ảnh hiện tại (bao gồm cả URL cũ và mới)
      if (images.length > 0) {
      ...
      if (response.data.success) {
        alertService.success('Cập nhật công thức thành công', 'Thành công').then(() => {
          router.back();
        });
      } else {
        alertService.error(response.data.message || 'Không thể cập nhật công thức', 'Lỗi');
      }
    } catch (error: any) {
      alertService.error(error.response?.data?.message || error.message || 'Không thể cập nhật công thức', 'Lỗi');
    } finally {
      setSaving(false);
    }
  };
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.content}>
            ...
            <View style={[styles.header, isDark && styles.headerDark]}>
              ...
              <ThemedText style={styles.headerTitle}>Chỉnh sửa công thức</ThemedText>
              <View style={{ width: 24 }} />
            </View>
                  ...
                  {CATEGORIES.map((cat) => (
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        isDark && !category && styles.chipDark,
                      ]}
                  ...
                  {DIFFICULTIES.map((diff) => (
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        isDark && !difficulty && styles.difficultyChipDark,
                      ]}
              ...
              >
                {saving ? (
                  ...
                  <>
                    ...
                    <ThemedText style={styles.submitBtnText}>Lưu thay đổi</ThemedText>
                  </>
                )}
              </TouchableOpacity>
            </View>
